...
CLAIM_PATTERNS = [
    ...
    r"(?i)\b(?:according to|study shows?|research (?:shows?|found))\b",
    r"(?i)\b(?:government|official|federal|provincial|state)\s+(?:data|statistics?)\b",
    r"(?i)\b(?:average|median|typical|standard)\s+(?:rate|fee|cost|salary)\b",
...
class FactCheckerAgent(BaseAgent):
    ...
    async def run(self, article_draft_path: str, output_dir: str = "outputs") -> dict[str, Any]:
        ...
        self.logger.info(
            ...
            f"{report['summary']['issues_count']} issues found"
        )
        return report

    async def _extract_claims(self, text: str) -> list[dict]:
        """Use regex + LLM to extract verifiable claims from text."""
        claims: list[dict] = []
        seen: set[str] = set()
    ...
    async def _llm_extract_claims(self, text: str) -> list[dict]:
        """Use LLM to find additional verifiable claims."""
        prompt = ("From the following article excerpt, identify up to 20 specific factual claims\n"
                  "that can be verified (statistics, fees, rates, timelines, regulatory facts).\n"
                  "Return JSON array: [{id, text, type}]\n\n"
    ...
    async def _verify_claim(self, claim: dict) -> dict:
        """Verify a single claim using search + source cross-referencing."""
        result = {
            **claim,
            "status": "UNVERIFIED",
            "confidence": 0.0,
    ...
    def _build_verification_query(self, claim: dict) -> str:
        ...
        trusted = " OR site:".join(TRUSTED_DOMAINS[:5])
    ...
    def _evaluate_search_results(self, claim_text: str, results: list[dict]) -> tuple:
        ...
        confidence = min(1.0, (trusted_hits / max(len(results), 1)) + len(supporting) * 0.1)
    ...
    async def _validate_statistics(self, text: str) -> dict:
        ...
        for rate in usd_cad:
            r = float(rate)
            if 1.0 <= r <= 2.0:
                stats["validated"].append({"type": "exchange_rate", "value": r})
            else:
                stats["flagged"].append({"type": "exchange_rate", "value": r,
                                         "note": f"USD/CAD={r} unusual"})
        interest_rates = re.findall(r"([\d.]+)%\s+(?:interest|APR|APY|annual)", text, re.I)
        for rate in interest_rates:
            r = float(rate)
            if r > 30:
                stats["flagged"].append({"type": "interest_rate", "value": r})
            else:
                stats["validated"].append({"type": "interest_rate", "value": r})
        return stats

    def _build_report(self, verified: list, url_results: dict,
                      stats: dict, start_time: datetime) -> dict:
