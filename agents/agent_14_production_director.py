...
class ProductionDirectorAgent(BaseAgent):
    ...
    async def _generate_html_report(self, production_data: Dict,
        ...
        # Pre-build agent status cards (avoids nested f-strings)
        agent_cards = "".join([
        ...
        # Pre-build error list HTML (avoids nested f-strings)
        if not errors:
            errors_html = "<p style='color: #28a745; font-weight: 600;'>No errors detected. All systems nominal.</p>"
        else:
            error_items = "".join([
                "<li class=\"error-" + str(e.get("severity", "medium")) + "\">"
                "<strong>" + str(e.get("type", "error")) + "</strong>: "
                + str(e.get("message", "")) + "</li>"
                for e in errors
            ])
            errors_html = "<ul class='error-list'>" + error_items + "</ul>"

        seo_ok = metrics.get("avg_seo_score", 0) >= 95
        eeat_ok = metrics.get("avg_eeat_score", 0) >= 95
        seo_class = "ready" if seo_ok else "rejected"
        seo_label = "PASS" if seo_ok else "FAIL"
        eeat_class = "ready" if eeat_ok else "rejected"
        eeat_label = "PASS" if eeat_ok else "FAIL"
...
<td class="{seo_class}">{seo_label}</td>
</tr>
<tr>
<td>EEAT Score</td>
<td>&ge; 95</td>
<td>{metrics.get("avg_eeat_score", 0)}</td>
<td class="{eeat_class}">{eeat_label}</td>
</tr>
</table>
</div>

<div class="card">
<h2>Errors &amp; Warnings ({len(errors)})</h2>
{errors_html}
</div>
