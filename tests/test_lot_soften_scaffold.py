...
def test_case1_parenthesis_removed_whole_text_outside_intact():
    out, _ = soften("build payment history (~35% of FICO score) while funding a certificate.")[0], None
...
def test_case2a_bold_only_number_removed():
    out = soften("the timeline shrinks to **6 percentage points** typically.")[0]
    assert "**" not in out


def test_case2b_bold_with_label_keeps_the_label():
    out = soften("**Keep utilization below 30%** for maximum impact.")[0]
    assert "**Keep utilization**" in out          # label kept, number gone, bold closed tight


def test_case3_trailing_unit_swallowed():
    out = soften("you make fixed monthly payments ($15/month), funds are held.")[0]
...
def test_case4_short_appositive_removed():
    out = soften("You deposit collateral —typically $200— which becomes your credit limit.")[0]
...
def test_guard_official_link_in_paren_is_never_deleted():
    out = soften("the window (30 days, see [FTC](https://consumer.ftc.gov/articles/disputing-errors-your-credit-reports)).")[0]
    assert "consumer.ftc.gov" in out              # the citation survives


def test_guard_long_appositive_is_kept_number_only_stripped():
    out = soften("history —the single largest FICO factor at 35 percentage points of your score— matters.")[0]
...
def test_appositive_threshold_is_configurable():
    ...
    kept = soften(s, max_appos_words=2)[0]         # stricter -> keep appositive, strip number only
    nuked = soften(s, max_appos_words=4)[0]        # default -> remove the short appositive
