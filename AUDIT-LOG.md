...
- **Vérifié après merge** : `gh api .../actions/workflows/294603021` confirme `"state": "active"` -- le fichier est accepté par GitHub. Prochains créneaux planifiés (06:00/13:00 UTC) à surveiller pour confirmer que "1 article/créneau" est maintenant réellement effectif (et pas seulement affiché) grâce au fix `needs.detect`.
- **Crons** : **actifs**, 2 runs/jour (06:00 et 13:00 UTC), draft-only, OPPORTUNITY forcé, **1 article/créneau réellement effectif depuis ce fix** (ne l'était pas avant, à cause du bug `needs.detect`).
